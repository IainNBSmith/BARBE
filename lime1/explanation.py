...
class Explanation(object):
    ...
    def as_html(self,
        ...
        out += u'''
        <div class="lime1 top_div" id="top_div%s"></div>
        ''' % random_id
        ...
        if self.mode == "classification" and predict_proba:
            ...
            var pp_div = top_div.append('div')
                                .classed('lime1 predict_proba', true);
            var pp_svg = pp_div.append('svg').style('width', '100%%');
            var pp = new lime1.PredictProba(pp_svg, %s, %s);
        ...
        if self.mode == "regression" and show_predicted_value:
            ...
            predict_value_js = u'''
                    var pp_div = top_div.append('div')
                                        .classed('lime1 predicted_value', true);
                    var pp_svg = pp_div.append('svg').style('width', '100%%');
                    var pp = new lime1.PredictedValue(pp_svg, %s, %s, %s);
                    ''' % (jsonize(float(self.predicted_value)),
                           jsonize(float(self.min_value)),
                           jsonize(float(self.max_value)))

        exp_js = '''var exp_div;
            var exp = new lime1.Explanation(%s);
        ''' % (jsonize([str(x) for x in self.class_names]))
        ...
        if self.mode == "classification":
            for label in labels:
                ...
                exp_js += u'''
                exp_div = top_div.append('div').classed('lime1 explanation', true);
        ...
        out += u'''
        <script>
        var top_div = d3.select('#top_div%s').classed('lime1 top_div', true);
        %s
        %s
        %s
        %s

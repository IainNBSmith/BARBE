...
def fit(df, class_name, columns, features_type, discrete, continuous,
    ...
    cmd = '../../LORE/yadt/dTcmd.exe -fd %s -fm %s -sep %s -d %s' % (
        data_filename, names_filename, sep, tree_filename)
    print("COMMAND PASSED: ", cmd.split())
    output = subprocess.run(cmd.split(), stderr=subprocess.STDOUT)

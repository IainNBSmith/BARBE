...
def fitness_sso(x0, bb, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
    ...
    #print("x0: ", x0)
    #print("idx_features: ", idx_features)
    #x0.remove(class_name)
    #x1.remove(class_name)
    ...
    ind_value = x0.index.ravel()
    #print("X Values...")
    ##print(ind_value)
    #print(x1)
    input_row0 = pd.DataFrame(columns=ind_value, index=[0])
    input_row0.iloc[0] = x0.to_numpy().reshape((1, -1))
    input_row1 = pd.DataFrame(columns=ind_value, index=[0])
    input_row1.iloc[0] = x1
    y0 = bb.predict(input_row0)[0]
    y1 = bb.predict(input_row1)[0]

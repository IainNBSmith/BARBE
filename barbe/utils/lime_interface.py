...
class LimeNewPert(LimeTabularExplainer):
    ...
    def predict(self, X):
        ...
        for c_label in self.base.used_features.keys():
            ...
            model_predictions.append(c_predictions)
    ...
    def slime(self,
        ...
        while not exit_loop and itert < 10:
            ...
            for k in range(1, num_features + 1):
                if (not flag) and (len(test_result) == (k+1) or test_result[k][0] < -tol):
                    flag = True
                    where_stop = k
            # fix unending loops
            original_num_samples = num_samples
            if num_samples != n_max:
                if np.isinf(test_result[where_stop][1]):
                    num_samples = 2 * num_samples
                else:
                    num_samples = min(int(test_result[where_stop][1]), 2 * num_samples)

                if num_samples <= 0:
                    num_samples = 2 * original_num_samples
                elif num_samples > n_max:
                    num_samples = n_max
            else:
                exit_loop = True
